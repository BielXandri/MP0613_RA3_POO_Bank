...
try {
    ...
    $bankAccount1->transaction(transaction: new DepositTransaction(amount: 150));
    pl('My new balance after deposit (+150) : ' . $bankAccount1->getBalance());

    // withdrawal -25
    pl('Doing transaction withdrawal (-25) with current balance ' . $bankAccount1->getBalance());
    $bankAccount1->transaction(transaction: new WithdrawTransaction(amount: 25));
    pl('My new balance after withdrawal (-25) : ' . $bankAccount1->getBalance());

    // withdrawal -600 (should fail)
    pl('Doing transaction withdrawal (-600) with current balance ' . $bankAccount1->getBalance());
    $bankAccount1->transaction(transaction: new WithdrawTransaction(amount: 600));
...
try {
    // create new account2 with balance 200 and silver overdraft (limit 100)
    $bankAccount2 = new BankAccount(200.0, new SilverOverdraft(100.0));
    pl('My initial balance : ' . $bankAccount2->getBalance());
    ...
    // withdrawal -300 (within overdraft limit)
    ...
    // withdrawal -50 (still within overdraft)
    ...
    // withdrawal -120 (exceeds overdraft limit → should fail)
...
try {
    // withdrawal -20 (should succeed if within overdraft)
...
try {
    // Close and reopen account 2
    $bankAccount2->closeAccount();
    pl('Account #2 is now closed.');

    $bankAccount2->reopenAccount();
    pl('Account #2 is reopened.');
} catch (BankAccountException $e) {
    pl($e->getMessage());
}
